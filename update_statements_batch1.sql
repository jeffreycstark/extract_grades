...
-- Class: EHSS-01, Term: 2022AT3E (searches year 2022)
-- Uses LIKE pattern matching to catch EHSS-01, EHSS-1, EHSS-01A, etc.
-- Only updates students with 2-3 records (normal enrollment, not repeats)
...
SET Grade = 'F'
WHERE ID = '12389'
  AND (NormalizedCourse LIKE 'EHSS-01%' OR NormalizedCourse LIKE 'EHSS-1%')
  AND parsed_termid LIKE '2022%'
  AND section NOT IN ('87', '147')
  AND Attendance = 'Normal'
  AND (
    SELECT COUNT(*) FROM AcademicCourseTakers AS t
    WHERE t.ID = '12389'
      AND (t.NormalizedCourse LIKE 'EHSS-01%' OR t.NormalizedCourse LIKE 'EHSS-1%')
      AND t.parsed_termid LIKE '2022%'
...
-- End of batch 1 (50 UPDATE statements)
